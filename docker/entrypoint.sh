...
# Only attempt to adjust users and use gosu if we are running as root
if [ "$(id -u)" = "0" ]; then
    echo "Running as root, adjusting permissions for PUID=$PUID and PGID=$PGID"
    ...
    echo "Running as non-root user $(id -u). Skipping permission adjustments."
    exec "$@"
fi
