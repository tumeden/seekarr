...
def _config_view(config: RuntimeConfig, store: StateStore) -> dict[str, Any]:
    ...
    def _instance_row(app: str, inst) -> dict[str, Any]:
        return {
            ...
            "cleanup_enabled": bool(getattr(inst, "cleanup_enabled", False)),
            "cleanup_dry_run": bool(getattr(inst, "cleanup_dry_run", False)),
            "cleanup_stuck_hours": int(getattr(inst, "cleanup_stuck_hours", 24) or 24),
            "cleanup_require_issue": bool(getattr(inst, "cleanup_require_issue", True)),
            "cleanup_remove_from_client": bool(getattr(inst, "cleanup_remove_from_client", True)),
            "cleanup_blocklist": bool(getattr(inst, "cleanup_blocklist", True)),
            "cleanup_skip_redownload": bool(getattr(inst, "cleanup_skip_redownload", False)),
...
def create_app(db_path: str | None = None) -> Flask:
    ...
    def _materialize_db_instance(
        ...
        return ArrSyncInstanceConfig(
            ...
            cleanup_enabled=_to_bool(row.get("cleanup_enabled"), False),
            cleanup_dry_run=_to_bool(row.get("cleanup_dry_run"), False),
            cleanup_stuck_hours=max(1, _to_int(row.get("cleanup_stuck_hours"), 24)),
            cleanup_require_issue=_to_bool(row.get("cleanup_require_issue"), True),
            cleanup_remove_from_client=_to_bool(row.get("cleanup_remove_from_client"), True),
            cleanup_blocklist=_to_bool(row.get("cleanup_blocklist"), True),
            cleanup_skip_redownload=_to_bool(row.get("cleanup_skip_redownload"), False),
            arr=ArrConfig(
                enabled=enabled,
        ...
        try:
            ...
            for row in inst_in:
                ...
                values = {
                    ...
                    "cleanup_enabled": 1 if bool(row.get("cleanup_enabled", False)) else 0,
                    "cleanup_dry_run": 1 if bool(row.get("cleanup_dry_run", False)) else 0,
                    "cleanup_stuck_hours": max(1, min(720, int(row.get("cleanup_stuck_hours") or 24))),
                    "cleanup_require_issue": 1 if bool(row.get("cleanup_require_issue", True)) else 0,
                    "cleanup_remove_from_client": 1 if bool(row.get("cleanup_remove_from_client", True)) else 0,
                    "cleanup_blocklist": 1 if bool(row.get("cleanup_blocklist", True)) else 0,
                    "cleanup_skip_redownload": 1 if bool(row.get("cleanup_skip_redownload", False)) else 0,
                    "arr_url": arr_url,
                }
